...
for index in range(0,282):
    ...
    plt.rcParams['font.size'] = 28  # font
    scatter = plt.scatter(umap_embedding[:, 0], umap_embedding[:, 1],label='Total',c='whitesmoke', s=100, alpha=0.7)
      ...
      if (c_folder[0][ground_truth_labels[number]] == "DMSO"):
        if count2 == 0:
    ...
    for number in range(0,len(cluster_result)):
      if ground_truth_labels[number] == index:
        ...
        drug_embedding.append((umap_embedding[number, 0], umap_embedding[number, 1]))
