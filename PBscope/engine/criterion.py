...
class CCLoss(nn.Module):
    def __init__(self, temperature=0.5,alpha=0.5, gamma=2):
    ...
    def focal_loss(self, logits, targets):
        ...
        target_probs = probs[range(len(probs)), targets]
        focal_weight = (1 - target_probs) ** self.gamma
        F_loss = self.alpha * focal_weight * ce_loss
        return F_loss.mean()
    ...
    def instance_loss(self, z1, z2):  
        ...
        #loss = self.focal_loss(logits, self.labels)
        return loss
    ...
    def cluster_loss(self, p1, p2):
        ...
        for k_ in range(k):
            ...
            #loss_ce.append(self.focal_loss(logits[k_], self.cluster_labels))
...
def clustering_accuracy_metrics(cluster_labels, ground_truth):
    ...
    metrics["mean_cluster_nmi"] = np.mean(cluster_nmis)
