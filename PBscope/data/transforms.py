...
from data.MCJitter import MCJitter
import copy
...
class CCTransforms:
    def __init__(self, crop_size=224, s=0.5, blur=False, validation=False,k = 1):
        ...
        if validation:
            ...
            augmentations = [T.RandomResizedCrop(size=crop_size),
                             ...
                             T.RandomApply([T.ColorJitter(0.4 * s, 0.4 * s, 0.4 * s, 0.2 * s)],p=0.8),
            ...
            augmentations1 = copy.deepcopy(augmentations)
            augmentations1 += [T.ToTensor()]
            self.transforms1 = T.Compose(augmentations1)
            
            augmentations2 = [T.RandomResizedCrop(size=crop_size),
                              T.RandomHorizontalFlip(),
                              ]
            
            augmentations2 = copy.deepcopy(augmentations)
            augmentations2.append(MCJitter(0.1*s, 0.1*s, 0.1*s))
            augmentations2 += [T.ToTensor()]
            self.transforms2 = T.Compose(augmentations2)
    ...
    def __call__(self, x):
        if self.validation:
            ...
            return torch.stack([self.transforms1(x), self.transforms1(x),self.transforms2(x),self.transforms2(x)])
