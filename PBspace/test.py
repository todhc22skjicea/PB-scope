...
output_dir = "/home/sdx/drug_model/result_T1"
base_path = "/home/sdx/drugdataset_final/dataset_t1/test"
print(f"Evaluating")
model = torch.load("/home/sdx/drug_model/model/T1/CC/model.pth")
...
for folder_index in range(0,class_number):     
    ...
    for p in range(0,k):
      ...
      #print('len',len(target.data.cpu().tolist()))
    ground_truth_labels += target.data.cpu().tolist()
    #print('feature',len(feature_extract)) 
    ...
    #print(correct)
    #print(preds.data.cpu()[0,0,:])

    #print(selected_numbers)

    #print(target.data.cpu())
    #print(torch.tensor(feature_extract))
...
umap_embedding = reducer.fit_transform(feature_extract)  

unique_clusters = np.unique(cluster_labels)
unique_drug = np.unique(ground_truth_labels)
cluster_centers = np.zeros((len(unique_clusters), 2))
drug_centers = np.zeros((len(unique_drug), 2))
for i in range(0,len(unique_clusters)):
    cluster_indices = np.where(np.array(cluster_labels) == i)[0]
    cluster_centers[i] = np.array(umap_embedding)[cluster_indices].mean(axis=0)
    
# 
distances = pairwise_distances(umap_embedding, cluster_centers, metric='euclidean')
#max_distance = np.max(distances)

similarities = 1 - distances*distances/ np.max(distances*distances)
average_similarities = np.zeros((len(np.unique(ground_truth_labels)), len(unique_clusters)))

for ground_truth_class in np.unique(ground_truth_labels):
    for cluster in unique_clusters:
        gt_indices = np.where(ground_truth_labels == ground_truth_class)[0]
        cluster_index = np.where(unique_clusters == cluster)[0][0]
        average_similarities[ground_truth_class, cluster_index] = np.mean(similarities[gt_indices, cluster_index])
#print(average_similarities)


for j in range(0,len(unique_drug)):
    drug_indices = np.where(np.array(ground_truth_labels) == j)[0]
    drug_centers[j] = np.array(umap_embedding)[drug_indices].mean(axis=0)
distances = pairwise_distances(drug_centers, cluster_centers, metric='euclidean')

#drug_centers_norm = np.linalg.norm(drug_centers, axis=1)[:, np.newaxis]
#cluster_centers_norm = np.linalg.norm(cluster_centers, axis=1)[:, np.newaxis]

#cosine_similarity = np.dot(drug_centers, cluster_centers.T) / (drug_centers_norm * cluster_centers_norm.T)
...
[drug_number, cluster_class] = average_similarities.shape
closest_indices = np.zeros((cluster_class, 5),dtype = 'uint8')
#print(average_similarities.shape)
for i in range(0,cluster_class):
    sorted_indices = np.argsort(average_similarities[:,i])
    #print(sorted_indices)
    closest_indices[i] = np.argsort(sorted_indices)[:5]
 
for i, indices in enumerate(closest_indices):
    print(f"cluster{i}:")
    for index in indices:
        print(f" - {folders_class[index]}") 

positive_indices = np.where(np.array(ground_truth_labels) == 279)[0]
positive_centers = np.array(umap_embedding)[positive_indices].mean(axis=0)
positive_distances = pairwise_distances(positive_centers, cluster_centers)
max_positive_distance = np.max(positive_distances)
positive_average_similarities = 1 - positive_distances / max_positive_distance
positive_closest_indices = np.zeros((1, 5),dtype = 'uint8')
positive_sorted_indices = np.argsort(positive_average_similarities[:,i])
#print(sorted_indices)
positive_closest_indices = np.argsort(positive_sorted_indices)[:5]
print(f"cluster{i}:")
for index in positive_closest_indices:
  print(f" - {folders_class[index]}") 
